...
def main():
    ...
    for t in range(n_steps):
        ...
        # ------------------ Top panel: signals -----------------------
        # Split features into "temperature-like" and others, so we can put temp on a second y-axis
        temp_feature = next((c for c in features if "temp" in c.lower()), None)
        primary_features = [c for c in features if c != temp_feature]
