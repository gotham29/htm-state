#!/usr/bin/env python3
"""
Live visualization for Demo 3 — Healthcare Operator Workload Monitoring

This reuses the same simple anomaly → state → spike pipeline as
offline_demo_healthcare.py, then streams it as a live plot.
"""
...
# ---------------------------------------------------------------------
# Spike / state helpers (mirrored from offline_demo_healthcare.py)
# ---------------------------------------------------------------------
...
def run_htm_state_pipeline_for_demo(
    ...
    z-score across a small set of motion/interaction features.
    Then:
      anomaly -> EMA -> state
    ...
    if missing:
        raise ValueError(
            f"Healthcare live demo: missing expected columns {missing}. "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Live HTM healthcare workload demo")
    parser.add_argument("--csv", required=True, help="Path to healthcare operator stream CSV")
    parser.add_argument(
        "--rate-hz",
        type=float,
        default=10.0,
...
def main():
    args = parse_args()
    print(f"Loading healthcare operator stream: {args.csv}")
    df = pd.read_csv(args.csv)
    print(f"Loaded {len(df)} timesteps.")

    features = ["motion_energy", "cursor_velocity", "interaction_density", "task_variability"]
    ...
    # For now we use the same synthetic HTM-State proxy as the offline demo
    ...
    window = 300  # number of points in rolling window

    buffer_feat: Dict[str, List[float]] = {col: [] for col in features}
    ...
    for t in range(len(df)):
        ...
        top_ax.set_title("Healthcare Signals")
        ...
        bot_ax.set_title("HTM-State (synthetic) + Detected Workload Transitions")
